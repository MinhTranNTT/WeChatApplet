  ...
  <div class="product-form">
    <el-form :model="productInfo" :rules="productRules" ref="productForm" label-width="120px" class="login-form"
      ...
      <el-form-item label="轮播图列表" prop="bannerList">
        <el-upload v-model:file-list="bannerList" :action="uploadUrl" list-type="picture-card"
      ...
      <el-form-item label="商品详情图图片" prop="detailList">
        <el-upload v-model:file-list="detailList" :action="uploadUrl" list-type="picture-card"
      ...
      <div class="commit">
        <el-button class="commit-button" type="primary" @click="submmitProduct">提交</el-button>
      </div>
    </el-form>
  </div>
</template>
...
export default {
  setup() {
    ...
    const handleBannerRemove = async(uploadFile, uploadFiles) => {
      ...
      await axios.post('/file/delete', { url: uploadFile.response })
      for (var i = 0; i < productInfo.bannerList.length; i++) {
        if (productInfo.bannerList[i].url == uploadFile.response) {
    ...
    const handleDetailRemove = async(uploadFile) =>{
      ...
      for (var i = 0;i<productInfo.detailList.length;i++){
        if(productInfo.detailList[i].url == uploadFile.response){
    ...
    const submmitProduct = async() => {
      try {
        productForm.value.validate((valid)=>{
          ...
          const { data: res } = axios.post('/products/add', productInfo)
        });
    ...
    return { productInfo, productForm, categoryList, uploadUrl, location, token, dialogImageUrl, dialogVisible, disabled,openDepartureLocationDialog, handlePictureCardPreview, onBannerSuccess,
      handleBannerRemove,handleDetailRemove,handlePosterSuccess,onDetailSuccess,handleChange,addDepartureMeetingPoint,deleteDepartureLocationCard,openReturnLocationDialog,addReturnMeetingPoint,deleteReturnLocationCard,
      productRules,submmitProduct}
  }
