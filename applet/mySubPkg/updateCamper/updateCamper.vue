<template>
	<view>
		...
		<view class="title">
			<view class="main-title">更新营员信息</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			async updateCampers(){
				const that=this;
				await uni.$http.put('/user/camper/update',that.camperInfo)
				var campersList =await uni.getStorageSync('campersList');
				for(var i =0;i<campersList.length;i++){
					if(campersList[i].id === that.camperInfo.id){
						campersList.splice(i,1);
						campersList.splice(i,0,that.camperInfo)
						await uni.setStorageSync("campersList",campersList);
					}
				}
			},
